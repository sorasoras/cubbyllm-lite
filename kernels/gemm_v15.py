"""v15: CHAMPION — sched-group-barrier batching + int8 epilogue. — all 12 fragment loads in
...
extern "C" __global__ void moe_v15(const uint32_t* __restrict__ Ap,
        ...
        for (int kw = 0; kw < Kw; kw += KCW) {
            ...
            for (int s = 0; s < KCW / 4; ++s) {
                ...
                for (int ng = 0; ng < 4; ++ng) {
                    ...
                    __builtin_amdgcn_sched_group_barrier(0x004, 1, 1);
...
def compile_src(src, tag):
    ...
    fn = ctypes.c_void_p()
    assert HIP.hipModuleGetFunction(ctypes.byref(fn), m2, b"moe_v15") == 0
    return fn
