"""v12: v7b + triple-buffered LDS (prefetch 2 chunks ahead) — hides the
L2 load latency otherwise exposed at each end-of-chunk barrier.
...
extern "C" __global__ void moe_v12(const uint32_t* __restrict__ Ap,
    ...
    for (int tile = blockIdx.x; tile < ntiles; tile += gridDim.x) {
        ...
        load(0, 0);
        if (Kw > KCW) load(KCW, 1);
        __syncthreads();
        int nc = (Kw + KCW - 1) / KCW;
        for (int c = 0; c < nc; ++c) {
            int* LA = lds + (c % 3) * BUFSZ;
            int* LB = LA + 128 * AST;
            ...
            // prefetch 2 chunks ahead into the buffer freed by compute(c-1)
            if (c + 2 < nc) load((c + 2) * KCW, (c + 2) % 3);
            __syncthreads();
        }
        int rbase = (lane >> 4) * 8;
...
def compile_src(src, tag):
    ...
    fn = ctypes.c_void_p()
    assert HIP.hipModuleGetFunction(ctypes.byref(fn), m2, b"moe_v12") == 0
    return fn
