"""v8: persistent INT4xINT4 grouped MoE, 128x256 block tiles (L2-reuse optimized).
...
extern "C" __global__ void moe_v8(const uint32_t* __restrict__ Ap,
        ...
        for (int kw = 0; kw < Kw; kw += KCW) {
            ...
            for (int s = 0; s < KCW / 4; ++s) {
                ...
                for (int ng = 0; ng < 8; ++ng) {
                    ...
                    acc[0 * 8 + ng] = __builtin_amdgcn_wmma_i32_16x16x32_iu4_w32_gfx12(1, a0, 1, b, acc[0 * 8 + ng], 0);
                    acc[1 * 8 + ng] = __builtin_amdgcn_wmma_i32_16x16x32_iu4_w32_gfx12(1, a1, 1, b, acc[1 * 8 + ng], 0);
                }
            }
...
def compile_src(src, tag):
    ...
    fn = ctypes.c_void_p()
    assert HIP.hipModuleGetFunction(ctypes.byref(fn), m2, b"moe_v8") == 0
    return fn
