"""v6: persistent single-launch INT4xINT4 grouped MoE for RDNA4 (gfx1201).

Block: 256 threads (8 warps). Each block computes 128x128 output tiles
(warp w -> 16-row band, NT=8 -> 128 cols). 64-k chunks, 2 WMMA calls per
...
#define KCW 8
#define AST 10
#define BST 256          // B stored word-PAIR-interleaved: pair p, col c at [p*256 + c*2]
#define BUFSZ (96 * AST + 4 * BST)
#define FLAGSZ 8
...
extern "C" __global__ void moe_v6(const uint32_t* __restrict__ Ap,
    ...
    for (int tile = blockIdx.x; tile < ntiles; tile += gridDim.x) {
        ...
        volatile int* produced = (volatile int*)(lds + 2 * BUFSZ);
        volatile int* consumed = produced + 2;
        volatile int* adone = consumed + 2;
        // producers: warp 6 loads A (96 rows), warp 7 loads B
        const bool is_prod = (warp >= 6);
        auto load_A = [&](int kw, int buf) {
            int* LA = lds + buf * BUFSZ;
            for (int w = lane; w < 96 * 4; w += 32) {
                int r = w >> 2, qq = w & 3;
                *(v2i*)(LA + r * AST + qq * 2) = *(const v2i*)(Aptr + (long)r * Kw + kw + qq * 2);
            }
        };
        auto load_B = [&](int kw, int buf) {
            int* LB = lds + buf * BUFSZ + 96 * AST;
            for (int w = lane; w < 4 * 128; w += 32) {
                int p = w >> 7, nl = w & 127;
                v2i val;
        ...
        if (threadIdx.x == 0 && threadIdx.y == 0) {   // LDS flags are per-block
            produced[0] = 0; produced[1] = 0; consumed[0] = 0; consumed[1] = 0;
            adone[0] = 0; adone[1] = 0;
        }
        // all warps cooperatively load chunk 0 (fast), then specialize
        {
            int* LA = lds;
            int* LB = LA + 96 * AST;
            for (int w = threadIdx.y * 32 + lane; w < 96 * 4; w += 256) {
                int r = w >> 2, qq = w & 3;
                *(v2i*)(LA + r * AST + qq * 2) = *(const v2i*)(Aptr + (long)r * Kw + qq * 2);
            }
            for (int w = threadIdx.y * 32 + lane; w < 4 * 128; w += 256) {
                int p = w >> 7, nl = w & 127;
                v2i val;
                val.x = Bptr[(long)(2 * p) * N + nl];
                val.y = Bptr[(long)(2 * p + 1) * N + nl];
                *(v2i*)(LB + p * BST + nl * 2) = val;
            }
            __syncthreads();
            if (threadIdx.x == 0 && threadIdx.y == 0) produced[0] = 1;
        }
        int nc = (Kw + KCW - 1) / KCW;
        if (is_prod) {
            for (int c = 1; c < nc; ++c) {
                int b = c & 1;
                int drains = 6 * (c / 2);                // buf b drained by chunks b..c-2
                while (*(volatile int*)&consumed[b] < drains) { }
                if (warp == 6) {
                    load_A(c * KCW, b);
                    __threadfence_block();
                    if (lane == 0) adone[b] = c + 1;      // A ready -> release warp 7
                } else {
                    while (*(volatile int*)&adone[b] < c + 1) { }
                    load_B(c * KCW, b);
                    __threadfence_block();
                    if (lane == 0) produced[b] = c + 1;   // full chunk ready
                }
            }
        } else {
            int cw = warp;                               // consumer id 0..5
            for (int c = 0; c < nc; ++c) {
                int b = c & 1;
                while (*(volatile int*)&produced[b] < c + 1) { }
                int* LA = lds + b * BUFSZ;
                int* LB = LA + 96 * AST;
                for (int s = 0; s < KCW / 4; ++s) {
                    ...
                    for (int ng = 0; ng < 4; ++ng) {
                        v2i bfrag = *(const v2i*)(LB + pb + ng * 32);
                        acc[0 * 4 + ng] = __builtin_amdgcn_wmma_i32_16x16x32_iu4_w32_gfx12(1, a0, 1, bfrag, acc[0 * 4 + ng], 0);
                        acc[1 * 4 + ng] = __builtin_amdgcn_wmma_i32_16x16x32_iu4_w32_gfx12(1, a1, 1, bfrag, acc[1 * 4 + ng], 0);
                    }
                }
                __threadfence_block();
                if (lane == 0) atomicAdd((int*)&consumed[b], 1);
            }
        }
        __syncthreads();
        int rbase = (lane >> 4) * 8;
...
def compile_src(src, tag):
    ...
    fn = ctypes.c_void_p()
    assert HIP.hipModuleGetFunction(ctypes.byref(fn), m2, b"moe_v6") == 0
    return fn
...
def build_tiles(seg_base, N):
    ...
    for eid, (base, rows, n_e) in enumerate(seg_base):
        if rows <= 0:
            continue
        for mt in range(rows // 96):
            for nc in range(nt):
                te.append(eid); tm.append(base + mt * 96); tn.append(nc * 128)
