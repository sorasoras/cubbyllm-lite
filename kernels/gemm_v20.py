"""v20: v19 + sched-group-barrier batching (9 loads grouped, then 8 WMMAs). (16 warps x 16 rows), KCW=4 chunks — the first
...
extern "C" __global__ void moe_v20(const uint32_t* __restrict__ Ap,
    ...
    for (int tile = blockIdx.x; tile < ntiles; tile += gridDim.x) {
        ...
        for (int kw = 0; kw < Kw; kw += KCW) {
            ...
            for (int ng = 0; ng < NT; ++ng) {
                bfr[ng] = *(const v2i*)(LB + kt * BST + col * 2 + ng * 32);
                __builtin_amdgcn_sched_group_barrier(0x100, 1, 0);
            }
            #pragma unroll
            for (int ng = 0; ng < NT; ++ng) {
                acc[ng] = __builtin_amdgcn_wmma_i32_16x16x32_iu4_w32_gfx12(1, a, 1, bfr[ng], acc[ng], 0);
...
def compile_src(src, tag):
    ...
    fn = ctypes.c_void_p()
    assert HIP.hipModuleGetFunction(ctypes.byref(fn), m2, b"moe_v20") == 0
    return fn
