"""v7: persistent INT4xINT4 grouped MoE, per-warp 4m x 4n register blocking.

Block: 128 threads (4 warps). Warp w -> 64x64 sub-tile (4 m-groups x
4 n-groups of WMMA tiles): A frags reused 4x, B frags reused 4x ->
128 B/WMMA LDS traffic (vs 288 in the 1m x 8n layout). 64-k chunks, 2 WMMA calls per
...
extern "C" __global__ void moe_v7(const uint32_t* __restrict__ Ap,
    ...
    for (int tile = blockIdx.x; tile < ntiles; tile += gridDim.x) {
        ...
        auto load = [&](int kw, int buf) {
            ...
            int* LB = LA + 128 * AST;
            for (int w = threadIdx.y * 32 + lane; w < 128 * 4; w += 128) {
                int r = w >> 2, qq = w & 3;
                *(v2i*)(LA + r * AST + qq * 2) = *(const v2i*)(Aptr + (long)r * Kw + kw + qq * 2);
            }
            for (int w = threadIdx.y * 32 + lane; w < 4 * 128; w += 128) {
                int p = w >> 7, nl = w & 127;
                v2i val;
        ...
        for (int kw = 0; kw < Kw; kw += KCW) {
            ...
            for (int s = 0; s < KCW / 4; ++s) {
                ...
                v2i a2 = *(const v2i*)(LA + (mrow0 + 32) * AST + 4 * s + 2 * kt);
                v2i a3 = *(const v2i*)(LA + (mrow0 + 48) * AST + 4 * s + 2 * kt);
                ...
                for (int ng = 0; ng < 4; ++ng) {
                    ...
                    acc[2 * 4 + ng] = __builtin_amdgcn_wmma_i32_16x16x32_iu4_w32_gfx12(1, a2, 1, b, acc[2 * 4 + ng], 0);
                    acc[3 * 4 + ng] = __builtin_amdgcn_wmma_i32_16x16x32_iu4_w32_gfx12(1, a3, 1, b, acc[3 * 4 + ng], 0);
                }
            }
        ...
        for (int mg = 0; mg < 4; ++mg)
            for (int ng = 0; ng < 4; ++ng)
                for (int j = 0; j < 8; ++j)
                    Out[(long)(mb + (warp >> 1) * 64 + mg * 16 + rbase + j) * N
...
def compile_src(src, tag):
    ...
    fn = ctypes.c_void_p()
    assert HIP.hipModuleGetFunction(ctypes.byref(fn), m2, b"moe_v7") == 0
    return fn
...
def launch_persistent(fn, Ap, Bt, tile_e, tile_m, tile_n, scale, Out, N, Kw, ntiles, P, block_y=4):
    ...
    st = HIP.hipModuleLaunchKernel(fn, P, 1, 1, 32, block_y, 1, SHARED, None, ptrs, None)
