"""v22: 256x256 block, 32 warps (1024 thr): warp = 16 rows x one 128-col
half (1m x 8n, acc 64). AI 512 FLOP/B at v19 wave residency. B chunk loads once for both halves. (16 warps x 16 rows), KCW=4 chunks — the first
...
extern "C" __global__ void moe_v22(const uint32_t* __restrict__ Ap,
    ...
    int rb = warp & 15; int ch = warp >> 4;
    int row_local = rb * 16 + col;   // 16 row-bands x 2 col-halves
    for (int tile = blockIdx.x; tile < ntiles; tile += gridDim.x) {
        ...
        for (int kw = 0; kw < Kw; kw += KCW) {
            ...
            int pb = kt * BST + ch * 256 + col * 2;   // col-half base (2 ints per column)
        ...
        for (int ng = 0; ng < NT; ++ng)
            for (int j = 0; j < 8; ++j)
                Out[(long)(mb + rb * 16 + rbase + j) * N + n0 + ch * 128 + ng * 16 + col] =
...
def compile_src(src, tag):
    ...
    fn = ctypes.c_void_p()
    assert HIP.hipModuleGetFunction(ctypes.byref(fn), m2, b"moe_v22") == 0
    return fn
