"""v5: NT=8 (16x128 per warp, 64x128 per block), K-chunk 64 (8 words).

vs v4: 2x block tile per B-load (B LDS reads per FLOP halve), half the
sync/load frequency, double-buffered. Same verified fragment mapping.
"""
import sys
...
NT = 8            # N-tiles of 16 per warp
KW_CHUNK = 8      # words per k-chunk = 64 k-elements
SRC = r"""
...
#define NT 8
#define KCW 8
// A: (M, Kw) packed; Bt: (Kw, N) transposed
// grid: (N/128, M/64), block: 32 x 4 (lane, warp); block tile 64 x 128
extern "C" __global__ void gemm_i4_v5(const uint32_t* __restrict__ Ap,
                                      const uint32_t* __restrict__ Bt,
                                      const float* __restrict__ scale,
                                      float* __restrict__ Out,
                                      int M, int N, int Kw) {
    extern __shared__ int lds[];   // 2 bufs x (64*9 A-padded + 8*128 B) ints
...
def compile_src(src, tag):
    ...
    fn = ctypes.c_void_p()
    assert HIP.hipModuleGetFunction(ctypes.byref(fn), m2, b"gemm_i4_v5") == 0
    return fn
...
if __name__ == "__main__":
    ...
    print(f"v5 grouped MoE (T={T}): sync={s1} memcpy={s2}  max|err| = {err:.1f}  {'PASS' if err == 0 else 'FAIL'}", flush=True)
    ...
    print(f"v5 int4 grouped-MoE : {t_moe:6.3f} ms  {gflop/t_moe:6.1f} TFLOPS")
    print(f"int8 _int_mm        : {t_i8:6.3f} ms  {gflop/t_i8:6.1f} TFLOPS")
    print(f"fp32 eager          : {t_f32:6.3f} ms  {gflop/t_f32:6.1f} TFLOPS")
    print(f"v5 vs fp32: {t_f32/t_moe:.2f}x | vs int8: {t_i8/t_moe:.2f}x | % of 663.5 peak: {gflop/t_moe/663.5*100:.1f}%")
