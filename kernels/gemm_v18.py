"""v18: split-wait scheduling — s_wait_dscnt 0x2 covers ng0-1 WMMAs
while ng2-3 fragment loads complete, then 0x0. — 6 ds_load_2addr_b64 with
...
extern "C" __global__ void moe_v18(const uint32_t* __restrict__ Ap,
    ...
    for (int tile = blockIdx.x; tile < ntiles; tile += gridDim.x) {
        ...
        for (int kw = 0; kw < Kw; kw += KCW) {
            ...
            __asm__ volatile("ds_load_2addr_b64 %0, %1 offset0:0 offset1:16"
                             : "=v"(fB3) : "v"(aB3));
            // split waits: fB0/fB1 (oldest 2 loads) ready first -> run ng0-1
            // WMMAs while fB2/fB3 complete, then wait for the rest
            __asm__ volatile("s_wait_dscnt 0x2" ::: "memory");
            const v2i a0 = a00, a1 = a10;
            const v2i b00 = fB0.xy, b01 = fB0.zw;
            const v2i b10 = fB2.xy, b11 = fB2.zw;
            #pragma unroll
            for (int ng = 0; ng < 2; ++ng) {
                const v2i b = (ng == 0) ? b00 : b01;
            ...
            const v2i b02 = fB1.xy, b03 = fB1.zw, b12 = fB3.xy, b13 = fB3.zw;
            #pragma unroll
            for (int ng = 2; ng < 4; ++ng) {
                const v2i b = (ng == 2) ? b02 : b03;
                acc[0 * 4 + ng] = __builtin_amdgcn_wmma_i32_16x16x32_iu4_w32_gfx12(1, a0, 1, b, acc[0 * 4 + ng], 0);
                acc[1 * 4 + ng] = __builtin_amdgcn_wmma_i32_16x16x32_iu4_w32_gfx12(1, a1, 1, b, acc[1 * 4 + ng], 0);
            }
            #pragma unroll
...
def compile_src(src, tag):
    ...
    fn = ctypes.c_void_p()
    assert HIP.hipModuleGetFunction(ctypes.byref(fn), m2, b"moe_v18") == 0
    return fn
