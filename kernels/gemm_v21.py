"""v21: v19 + 2m x 4n warp blocking (32x64 warp tiles): halves per-warp
B LDS traffic -> LDS drops from 112% of VALU to 75% (the 16-warp re-read fix). (16 warps x 16 rows), KCW=4 chunks — the first
...
extern "C" __global__ void moe_v21(const uint32_t* __restrict__ Ap,
    ...
    int mrow0 = (warp >> 1) * 32 + col;      // 2m x 4n warp tiling: 8 m-bands x 2 n-bands
    for (int tile = blockIdx.x; tile < ntiles; tile += gridDim.x) {
        ...
        for (int kw = 0; kw < Kw; kw += KCW) {
            ...
            int* LB = LA + 256 * AST;
                v2i a0 = *(const v2i*)(LA + mrow0 * AST + 2 * kt);
            v2i a1 = *(const v2i*)(LA + (mrow0 + 16) * AST + 2 * kt);
            int nb = (warp & 1) * 128 + col * 2;    // nband: cols 0-63 / 64-127
            for (int ng = 0; ng < 4; ++ng) {
                v2i b = *(const v2i*)(LB + kt * BST + nb + ng * 32);
...
def compile_src(src, tag):
    ...
    fn = ctypes.c_void_p()
    assert HIP.hipModuleGetFunction(ctypes.byref(fn), m2, b"moe_v21") == 0
    return fn
...
def launch_persistent(fn, Ap, Bt, tile_e, tile_m, tile_n, scale, Out, N, Kw, ntiles, P):
    ...
    st = HIP.hipModuleLaunchKernel(fn, P, 1, 1, 32, 16, 1, SHARED, None, ptrs, None)
...
def build_tiles(seg_base, N):
    ...
    for eid, (base, rows, n_e) in enumerate(seg_base):
        ...
        for mt in range(rows // 256):
            for nc in range(nt):
                te.append(eid); tm.append(base + mt * 256); tn.append(nc * 128)
