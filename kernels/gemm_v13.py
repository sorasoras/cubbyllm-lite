"""v13: v7b + ISA-guided scheduling: all 12 chunk-fragment loads hoisted
above the WMMA block — one s_wait_dscnt per chunk instead of six.
...
extern "C" __global__ void moe_v13(const uint32_t* __restrict__ Ap,
    ...
    for (int tile = blockIdx.x; tile < ntiles; tile += gridDim.x) {
        ...
        for (int kw = 0; kw < Kw; kw += KCW) {
            ...
            // hoist every fragment load for this chunk: issue all 12 b64 LDS
            // reads, take the dscnt wait once, then run 16 WMMAs unbroken
            v2i a0s[KCW / 4], a1s[KCW / 4], bfr[KCW / 4][4];
            #pragma unroll
            for (int s = 0; s < KCW / 4; ++s) {
                a0s[s] = *(const v2i*)(LA + mrow0 * AST + 4 * s + 2 * kt);
                a1s[s] = *(const v2i*)(LA + (mrow0 + 16) * AST + 4 * s + 2 * kt);
                #pragma unroll
                for (int ng = 0; ng < 4; ++ng)
                    bfr[s][ng] = *(const v2i*)(LB + (2 * s + kt) * BST + nb + ng * 32);
            }
            #pragma unroll
            for (int s = 0; s < KCW / 4; ++s) {
                #pragma unroll
                for (int ng = 0; ng < 4; ++ng) {
                    acc[0 * 4 + ng] = __builtin_amdgcn_wmma_i32_16x16x32_iu4_w32_gfx12(1, a0s[s], 1, bfr[s][ng], acc[0 * 4 + ng], 0);
                    acc[1 * 4 + ng] = __builtin_amdgcn_wmma_i32_16x16x32_iu4_w32_gfx12(1, a1s[s], 1, bfr[s][ng], acc[1 * 4 + ng], 0);
                }
            }
...
def compile_src(src, tag):
    ...
    fn = ctypes.c_void_p()
    assert HIP.hipModuleGetFunction(ctypes.byref(fn), m2, b"moe_v13") == 0
    return fn
