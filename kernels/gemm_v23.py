"""v23: v22 + 2m x 4n warp blocking (32x64 warp tiles, 8 m x 4 n bands):
per-warp loads 6/chunk (vs 9) -> LDS 75% of VALU (vs 100%). (16 warps x 16 rows), KCW=4 chunks — the first
341-FLOP/B tile that fits the LDS/occupancy budget (16.4 KB/CTA, 2 CTAs/CU).
...
#define KCW 4
#define AST 6
#define BST 512          // 256 cols x 2 ints per pair-row
#define WARPS 32
#define BUFSZ (256 * AST + 2 * BST)
extern "C" __global__ void moe_v23(const uint32_t* __restrict__ Ap,
    ...
    int warp = threadIdx.y & 31;
    int col = lane & 15, kt = lane >> 4;
    int mband = warp >> 2, nband = warp & 3;
    int row_local = mband * 32 + col;   // 8 m-bands x 4 n-bands
    for (int tile = blockIdx.x; tile < ntiles; tile += gridDim.x) {
        ...
        auto load = [&](int kw, int buf) {
            ...
            int* LB = LA + 256 * AST;
            for (int w = threadIdx.y * 32 + lane; w < 256 * 2; w += 1024) {
                int r = w >> 1, qq = w & 1;
                *(v2i*)(LA + r * AST + qq * 2) = *(const v2i*)(Aptr + (long)r * Kw + kw + qq * 2);
            }
            for (int w = threadIdx.y * 32 + lane; w < 2 * 256; w += 1024) {
                int p = w >> 8, nl = w & 255;
                v2i val;
        ...
        for (int kw = 0; kw < Kw; kw += KCW) {
            ...
            v2i a0 = *(const v2i*)(LA + row_local * AST + 2 * kt);
            v2i a1 = *(const v2i*)(LA + (row_local + 16) * AST + 2 * kt);
            int pb = kt * BST + nband * 128 + col * 2;   // 2 ints per column
        ...
        for (int mg = 0; mg < 2; ++mg)
            for (int ng = 0; ng < 4; ++ng)
                for (int j = 0; j < 8; ++j)
                    Out[(long)(mb + mband * 32 + mg * 16 + rbase + j) * N
                        + n0 + nband * 64 + ng * 16 + col] =
...
def compile_src(src, tag):
    ...
    fn = ctypes.c_void_p()
    assert HIP.hipModuleGetFunction(ctypes.byref(fn), m2, b"moe_v23") == 0
    return fn
...
def launch_persistent(fn, Ap, Bt, tile_e, tile_m, tile_n, scale, Out, N, Kw, ntiles, P):
    ...
    st = HIP.hipModuleLaunchKernel(fn, P, 1, 1, 32, 32, 1, SHARED, None, ptrs, None)
...
def build_tiles(seg_base, N):
    ...
    for eid, (base, rows, n_e) in enumerate(seg_base):
        if rows <= 0:
            continue
        for mt in range(rows // 256):
            for nc in range(nt):
                te.append(eid); tm.append(base + mt * 256); tn.append(nc * 256)
...
if __name__ == "__main__":
    ...
    HIP.hipMemcpy(host.ctypes.data_as(ctypes.c_void_p), ctypes.c_void_p(Out.data_ptr()), M_pad * N, 2)
    Out_np = host.reshape(M_pad, N).astype(np.float32)
