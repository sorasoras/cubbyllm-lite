"""v10: 256x128 blocks: two sequential m-passes per warp share each B
chunk (B LDS re-read x2, A LDS 256 rows) -> L2 AI 341 FLOP/B at 64-reg acc.
...
extern "C" __global__ void moe_v10(const uint32_t* __restrict__ Ap,
    ...
    for (int tile = blockIdx.x; tile < ntiles; tile += gridDim.x) {
        ...
        auto load = [&](int kw, int buf) {
            ...
            int* LB = LA + 256 * AST;
            for (int w = threadIdx.y * 32 + lane; w < 256 * 4; w += 256) {
                int r = w >> 2, qq = w & 3;
        ...
        for (int kw = 0; kw < Kw; kw += KCW) {
            ...
            for (int pass = 0; pass < 2; ++pass) {
                int mrow = pass * 128 + warp * 16 + col;
                for (int s = 0; s < KCW / 4; ++s) {
                    v2i a0 = *(const v2i*)(LA + mrow * AST + 4 * s + 2 * kt);
                ...
                for (int ng = 0; ng < 8; ++ng)
                    for (int j = 0; j < 8; ++j)
                        Out[(long)(mb + pass * 128 + warp * 16 + rbase + j) * N
                            + n0 + ng * 16 + col] =
                            (float)acc[ng][j] * scale[0];
                if (pass == 0) for (int i = 0; i < 8; ++i) acc[i] = {};
            }
...
def compile_src(src, tag):
    ...
    fn = ctypes.c_void_p()
    assert HIP.hipModuleGetFunction(ctypes.byref(fn), m2, b"moe_v10") == 0
    return fn
