"""v16: inline-asm hand-scheduled chunk body — 6 ds_load_2addr_b64 with
immediate offsets, ONE s_waitcnt, then 16 WMMAs. Manual VGPR plan ~127. — all 12 fragment loads in
one group, all 16 WMMAs in the next (AMD MFMA-loop scheduling technique).
...
extern "C" __global__ void moe_dbg(const uint32_t* __restrict__ Ap,
                                  ...
                                  int N, int Kw, int ntiles, int dbg_flag) {
    ...
    for (int tile = blockIdx.x; tile < ntiles; tile += gridDim.x) {
        ...
        for (int kw = 0; kw < Kw; kw += KCW) {
            ...
            // hand-scheduled chunk body (manual VGPR plan, ~127 regs):
            // 6 ds_load_2addr_b64 (volatile: LLVM may not interleave), one
            // explicit s_waitcnt lgkmcnt(0), then 16 WMMAs back-to-back.
            uint32_t aA0 = (uint32_t)(mrow0 * AST + 2 * kt) * 4;
            uint32_t aA1 = (uint32_t)((mrow0 + 16) * AST + 2 * kt) * 4;
            uint32_t aB0 = (uint32_t)(kt * BST + nb) * 4;
            uint32_t aB1 = (uint32_t)((kt + 2) * BST + nb) * 4;
            uint32_t aB2 = aB0 + 256, aB3 = aB1 + 256;   // ds offsets are 8-bit
            ...
            // DEBUG DUMP: fragment base + 24 values per lane
            if (Dbg != nullptr) {
                long long o = ((long long)blockIdx.x * 256 + threadIdx.y * 32 + lane) * 25;
                Dbg[o] = aA0; Dbg[o+1] = aA1; Dbg[o+2] = aB0; Dbg[o+3] = aB2;
                Dbg[o+4] = aB1; Dbg[o+5] = aB3;
                const int* fv = (const int*)&fA0;
                for (int q = 0; q < 4; ++q) Dbg[o+6+q] = fv[q];
                fv = (const int*)&fA1; for (int q = 0; q < 4; ++q) Dbg[o+10+q] = fv[q];
                const v4i* fb[4] = {&fB0, &fB1, &fB2, &fB3};
                for (int t = 0; t < 4; ++t) {
                    fv = (const int*)fb[t];
                    for (int q = 0; q < 4; ++q) Dbg[o+14+t*4+q] = fv[q];
                }
                return;
            }
            ...
            // call 1: A word-pair 2kt+4, B pair-row kt+2
            #pragma unroll
            for (int ng = 0; ng < 4; ++ng) {
...
def compile_src(src, tag):
    ...
    fn = ctypes.c_void_p()
    assert HIP.hipModuleGetFunction(ctypes.byref(fn), m2, b"moe_v16") == 0
    return fn
...
def build_tiles(seg_base, N):
    ...
    for eid, (base, rows, n_e) in enumerate(seg_base):
        ...
        for mt in range(rows // 128):
            for nc in range(nt):
                te.append(eid); tm.append(base + mt * 128); tn.append(nc * 128)
